...
export async function analyzeAppScreenshots(
  ...
  const clientResult = getAnthropicClient();
  if (!clientResult.success) {
    return { success: false, error: (clientResult as any).error };
  }
  const client = clientResult.data;
  ...
  for (let i = 0; i < screenshotsToAnalyze.length; i++) {
    ...
    if (!imageResult.success) {
      const errorMessage = (imageResult as any).error.message;
      failedImages.push({
        index: i,
        url: screenshot.url,
        error: errorMessage,
  ...
  if (!apiResult.success) {
    return { success: false, error: (apiResult as any).error };
  }
  ...
  if (appId) {
    ...
    try {
      ...
      if (storeResult.success) {
        ...
        console.warn(`[analyzeAppScreenshots] Failed to store analysis in cache: ${(storeResult as any).error.message}`);
      }
    } catch (cacheError) {
